    ...
    <div className="min-h-screen">
      ...
      <section className="py-20 lg:py-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {articles.length > 0 && (
            <div className="mb-16">
              <h2 className="font-heading text-2xl font-bold text-brand-ink mb-8">
                News
              </h2>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {articles.map((a) => (
                  <article
                    key={a.id}
                    className="bg-white border border-brand-line rounded-2xl overflow-hidden shadow-soft-sm flex flex-col"
                  >
                    {a.image ? (
                      ...
                      <div className="h-44 sd-hero-surface flex items-center justify-center">
                        <Newspaper className="w-10 h-10 text-brand-yellow/50" />
                      </div>
                    )}
                    <div className="p-6 flex flex-col flex-1">
                      <h3 className="font-heading text-lg font-bold text-brand-ink mb-3 leading-snug">
                        {a.title}
                      </h3>
                      <p className="text-sm text-brand-muted leading-relaxed mb-4 flex-1">
                        {a.excerpt || a.content}
                      </p>
                      <div className="flex flex-wrap items-center gap-4 pt-4 border-t border-brand-line text-xs text-gray-400">
                        {a.publishedAt && (
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {events.map((ev) => (
                  <div
                    key={ev.id}
                    className="bg-white border border-brand-line rounded-2xl p-6 shadow-soft-sm"
                  >
