...
export async function PUT(
  ...
  try {
    ...
    const wasActive = teacher.user.isActive;
    ...
    await logActivity(
      ...
      "TEACHER_UPDATE",
      `Updated teacher ${updatedTeacher.firstName} ${updatedTeacher.lastName}`,
      clientIp(req)
    );
    ...
    if (password) {
      ...
      await sendEmail({
        type: "PASSWORD_RESET",
        to: updatedTeacher.user.email,
        subject,
        html,
        refId: updatedTeacher.id,
...
export async function DELETE(
  ...
  try {
    ...
    await logActivity(
      (session.user as any).id,
      "TEACHER_DELETE",
      `Deleted teacher ${teacher.firstName} ${teacher.lastName}`,
      clientIp(req)
    );
